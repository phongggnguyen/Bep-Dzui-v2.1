...
export default function AnalyzePage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        // Keep pure base64 if needed, but img src works with data uri
  ...
  const handleAnalyze = async () => {
    ...
    setLoading(true);
    try {
      // Remove header "data:image/jpeg;base64," for API if present
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">AI Phân tích món ăn</h1>

      {/* Upload Area */}
      <div
        className="border-2 border-dashed border-orange-300 rounded-3xl p-8 text-center bg-white cursor-pointer hover:bg-orange-50 transition-colors"
      ...
      {image && !result && (
        <button
          ...
          disabled={loading}
          className="w-full mt-6 bg-orange-500 text-white py-3 rounded-xl font-bold text-lg shadow-lg hover:bg-orange-600 disabled:bg-gray-300 flex justify-center items-center gap-2 transition-all"
        >
      ...
      {/* Results */}
      {result && (
        <div className="mt-8 bg-white rounded-3xl p-6 shadow-sm border border-orange-100 animate-fade-in-up">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-bold text-green-600">{result.dishName}</h2>
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {result.ingredients.map((ing, idx) => (
                <span key={idx} className="bg-gray-100 text-gray-700 px-3 py-1 rounded-lg text-sm border border-gray-200">
                  {ing}
                </span>
              ))}
            </div>
