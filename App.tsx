...
import { Home, Camera, ChefHat, CalendarDays, User, ShoppingBag, Menu, LogOut, LogIn } from 'lucide-react';
...
// Default user profile (will be used for guests or as a fallback)
...
export default function App() {
  const { currentUser, logout, userProfile: authUserProfile, saveProfile: authSaveProfile } = useAuth();
        ...
        <div className="max-w-7xl mx-auto sm:flex min-h-screen">
          ...
          <aside className="hidden sm:flex flex-col w-72 h-screen sticky top-0 p-6">
             ...
             <nav className="flex-1 flex flex-col gap-3 bg-white/60 backdrop-blur-sm rounded-3xl p-4 shadow-sm border border-white/50 overflow-y-auto">
                ...
                {currentUser && ( // Show meal plan only if logged in
                ...
                {currentUser ? (
                  <>
                    ...
                    <button onClick={logout} className="flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all duration-300 group text-gray-600 hover:bg-red-50 hover:text-red-600">
                      <LogOut size={20} />
